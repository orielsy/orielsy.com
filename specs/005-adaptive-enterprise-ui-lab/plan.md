...
5. Embed the application into `src/content/research/documentation-driven-adaptive-ux.mdx` only after the resolver boundary is stable.
6. Keep the article unpublished unless Orielsy explicitly approves publication.
7. Do not add AI, WebLLM, MCP, backend infrastructure, persistence, telemetry, or a separate repository in Phase 1.
8. Do not publish production from the feature branch.
...
10. Embed it into the existing unpublished Research entry.
11. Run build/verification where execution access permits.
12. Review branch diff against `main` for unrelated changes.
