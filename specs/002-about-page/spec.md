...
### 2.9 Direct Contact
- Direct email belongs in the existing About → Connect experience and may also appear as a lightweight persistent footer action; do not create a Contact page, Contact nav item, contact form, CAPTCHA, modal, or reveal puzzle for the current contact model.
...
- The complete public email address should not appear in the initial rendered HTML on any contact surface. Assemble the address from separately stored components with the smallest reasonable amount of client-side JavaScript when an Email control is activated, then open a `mailto:` URL.
...
- If this public alias changes later, update the spec and the separately stored address components in every direct-email surface together.
