...
export const KNOWLEDGE: KnowledgeItem[] = [
  ...
  },
  {
    ...
    conditions: {
      executionMode: 'distributed',
      workerGroupMissing: true,
    },
    source: {
      type: 'release-note',
      label: 'Version 4.2 distributed execution changes',
    },
  },
  {
...
function matchesConditions(
  ...
  if (
    conditions.executionMode !== undefined &&
    conditions.executionMode !== input.config.executionMode
  ) {
    return false;
  }

  if (
    conditions.workerGroupMissing !== undefined &&
    conditions.workerGroupMissing !== facts.workerGroupMissing
  ) {
    return false;
  }

  if (
    conditions.targetEnvironment !== undefined &&
    conditions.targetEnvironment !== input.config.targetEnvironment
  ) {
    return false;
  }

  if (
    conditions.availableWorkerGroupCount !== undefined &&
    conditions.availableWorkerGroupCount !== facts.availableWorkerGroupCount
  ) {
    return false;
  }
...
export function resolveApplicableKnowledge(
  ...
  return input.knowledge.filter((item) => {
    const audienceMatches =
      item.audience === 'all' || item.audience === input.runtime.userFamiliarity;

    return (
      audienceMatches &&
      matchesVersion(item, input.runtime.productVersion) &&
      matchesConditions(item, input, facts)
    );
  });
}
